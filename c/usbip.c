...
int handle_get_descriptor(int sockfd, StandardDeviceRequest * control_req, USBIP_RET_SUBMIT *usb_req)
{
  ...
  if(control_req->wValue1 == 0x1) // Device
  {
   ...
   if(control_req->wValue1 == 0x3) // string
   {
     char str[255];
     int i;
     /* Bounds guard: only serve known low string indices; stall anything else
      * (notably Windows' one-shot MS-OS string-descriptor probe at index 0xEE).
      * Examples must keep strings[] in-bounds for every index their descriptors
      * reference; entries may be NULL to force a stall. */
     if(control_req->wValue0 >= 12 || strings[control_req->wValue0] == NULL)
     {
       printf("String (index %u unsupported -> STALL)\n",control_req->wValue0);
       handled = 1;
       send_usb_req(sockfd,usb_req,"",0,1);   // STALL
       return handled;
     }
     memset(str,0,255);
